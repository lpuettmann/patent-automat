function tests = test_count_nr_patents
...
function testIndicatorFind(testCase)
    ...
    [indic_find, ~, ~] = count_nr_patents(file_str, find_str);
...
function testNrFind(testCase)
    ...
    [~, nr_find, ~] = count_nr_patents(file_str, find_str);
    actSolution = nr_find;
    expSolution = 3;    
...
function testIxFind(testCase)
    ...
    [~, ~, ix_find] = count_nr_patents(file_str, find_str);
    actSolution = ix_find;
    expSolution = [2; 4; 6];    
...
function testEmptySearchstring(testCase)
    ...
    % The file count_nr_patents will issue a warning for such a short
