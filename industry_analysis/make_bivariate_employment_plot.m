function make_bivariate_employment_plot(fyr_start, fyr_end, industry_sumstats, ...
...
%     end
    ...
    plot(single_automat_ishare(indic_dot:end), ishare_manuf(indic_dot:end), ...
        'Color', color1_pick, 'Marker', marker_pick, ...
        'MarkerFaceColor', color1_pick, 'MarkerSize', marker_size, 'LineWidth', line_width)
    hold on
    hStartDot = plot(single_automat_ishare(1:indic_dot), ishare_manuf(1:indic_dot), 'Color', color1_pick, ...
...
% Export to pdf
print(figureHandle, 'output/bivariate_autompat_vs_employment.pdf', '-dpdf', '-r0')
