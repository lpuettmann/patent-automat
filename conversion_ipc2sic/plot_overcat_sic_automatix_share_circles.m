function plot_overcat_sic_automatix_share_circles(aggr_automix_share, ...
...
for i=1:length(sic_overcategories.letter)
    ...
    if i==pick_hl
        hs = scatter(year_start:year_end-1, aggr_automix_share(1:end-1, i), ...
            aggr_automix(1:end-1, i) .^ (7/10) , ...
            'MarkerEdgeColor', color_pick);
    end
    
    hold on
    hl = plot(year_start:year_end-1, aggr_automix_share(1:end-1, i), ...
        'Color', color_pick, 'LineWidth', 1.3);
...
annotation(figureHandle, 'textbox', ...
    [0.224158757549612 0.691860465116279 0.504918032786885 0.203488372093023], ...
...
% Export to pdf
% -----------------------------------------------------------------------
print_pdf_name = horzcat('output/overcat_sic_automatix_share_circle_', ...
